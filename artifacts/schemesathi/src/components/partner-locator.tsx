...
import { translatePartner, translatePartnerReason, useLanguage } from '../i18n';
...
export default function PartnerLocator() {
  ...
  const selectedLocalized = selected ? translatePartner(lang, selected) : undefined;
  ...
  return <div className="mx-auto max-w-6xl px-5 py-10 lg:px-10 lg:py-14">
    ...
    {data && <div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="order-1 space-y-5 lg:order-2">
        {selected && selectedLocalized && <article className="rounded-2xl border border-[hsl(var(--primary)/.4)] bg-[hsl(var(--primary))] p-6 text-[hsl(var(--primary-foreground))] shadow-lg"><p className="text-xs font-bold uppercase tracking-[.16em] opacity-70">{t('recommendedPartner')}</p><h2 className="mt-3 font-display text-2xl font-bold">{selectedLocalized.name}</h2><p className="mt-1 text-sm opacity-75">{selectedLocalized.type} · {selected.distanceKm} km · {t('score')} {selected.score}/100</p><div className="mt-5 grid gap-2 text-sm">{selected.reasons.map(reason => <span key={reason}>✓ {translatePartnerReason(lang, reason).replace(`${selected.distanceKm} km from your location`, t('distanceFromLocation').replace('{distance}', String(selected.distanceKm)))}</span>)}</div><div className="mt-5 rounded-xl bg-[hsl(var(--primary-foreground)/.1)] p-3 text-xs leading-5"><strong>{t('why')} </strong>{selected.eligibilityReasons.map(reason => translatePartnerReason(lang, reason)).join(' · ')}</div><div className="mt-6 flex flex-wrap gap-3"><a href={directions} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-[hsl(var(--primary-foreground))] px-4 py-3 text-sm font-bold text-[hsl(var(--primary))]"><Navigation size={15} /> {t('getDirections')}</a><Link href={`/application?scheme=${schemeId}&partner=${selected.id}`} className="inline-flex items-center gap-2 rounded-xl border border-[hsl(var(--primary-foreground)/.3)] px-4 py-3 text-sm font-bold">{t('applyHere')} <ArrowRight size={15} /></Link></div></article>}
        <div><div className="mb-3 flex items-center justify-between"><h2 className="font-display text-xl font-bold">{t('eligiblePartners')}</h2><span className="text-xs font-semibold text-[hsl(var(--muted-foreground))]">{eligible.length} {t('canReceive')}</span></div><div className="space-y-3">{eligible.map(partner => { const localized = translatePartner(lang, partner); return <button type="button" key={partner.id} onClick={() => setSelectedId(partner.id)} className={`w-full rounded-2xl border bg-[hsl(var(--card))] p-4 text-left ${selectedId === partner.id ? 'border-[hsl(var(--primary))] ring-2 ring-[hsl(var(--primary)/.12)]' : 'border-[hsl(var(--card-border))]'}`}><div className="flex items-start justify-between gap-3"><div><h3 className="font-display font-bold">{localized.name}</h3><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{localized.type} · {partner.distanceKm} km · {t('score')} {partner.score}</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${partner.status === 'ELIGIBLE' ? 'bg-[hsl(var(--success)/.14)] text-[hsl(var(--success-foreground))]' : 'bg-[hsl(var(--warning)/.16)] text-[hsl(var(--warning-foreground))]'}`}>{partner.status === 'ELIGIBLE' ? t('eligible') : t('limited')}</span></div><p className="mt-3 text-xs text-[hsl(var(--muted-foreground))]">{t('fundUtilization')} {partner.fundUtilizationPercent}% · {t('npa')} {partner.npaStatus === 'WITHIN_THRESHOLD' ? t('npaWithin') : t('npaAbove')} · {t('processingCapacity')} {partner.processingCapacityStatus === 'AVAILABLE' ? t('capacityAvailable') : t('limited')}</p></button>; })}</div></div>
        {data.partners.some(partner => !partner.eligible) && <details className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4"><summary className="cursor-pointer text-sm font-bold">{t('viewUnavailable')}</summary><div className="mt-4 space-y-3">{data.partners.filter(partner => !partner.eligible).map(partner => <div key={partner.id} className="rounded-xl bg-[hsl(var(--secondary))] p-3 text-sm"><strong>{translatePartner(lang, partner).name}</strong><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{partner.eligibilityReasons.map(reason => translatePartnerReason(lang, reason)).join(' · ')}</p></div>)}</div></details>}
      </div>
    </div>}
